...
def run_test(fin):
    """
    This function creates a list of examples data (used to learn the d-tree)
    ...
    data = []
    for line in lines:
        data.append(dict(list(zip(attributes,
...
def print_tree(tree, delimiter):
    ...
    """
    if type(tree) == dict:
        print("%s%s" % (delimiter, list(tree.keys())[0]))
        for item in list(tree.values())[0].keys():
            print("%s\t%s" % (delimiter, item))
            print_tree(list(tree.values())[0][item], delimiter + "\t")
    else:
        print("%s\t->\t%s" % (delimiter, tree))
